...
public class UIInventory : MonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		_onObjectPickUp.OnEventRaised += UpdateItemUI;
		_removeItemEvent.OnEventRaised += UpdateItemUI;
	...
	public void FillInventory(InventoryTabType _selectedTabType = InventoryTabType.Default, bool isNearPot = false)
	{
		...
		if (_selectedTab != null)
		{
			...
			//foreach (var s in _tabTypesList) { Debug.Log(s); }
	...
	public void InspectItem(ItemSO itemToInspect)
	{
		if (_availableItemSlots.Exists(o => o._currentItem.Item == itemToInspect))
		{
			...
			if (itemToInspect.ItemType.ActionType == ItemInventoryActionType.Use)
			{
	...
	void OnActionButtonClicked()
	{
		...
		if (_availableItemSlots.Count > selectedItemId && selectedItemId > -1)
		{
			...
			Debug.Log("YYY_测试OnActionButtonClicked");
			//check the selected Item type
			//call action function depending on the itemType
			switch (itemToActOn.ItemType.ActionType)
			{

				case ItemInventoryActionType.DoNothing:
					Debug.Log("YYY_OnActionButtonClicked使用珠子。itemToActOn"+ itemToActOn);
	...
	public void UpdateItemUI(ItemSO item)
    {
