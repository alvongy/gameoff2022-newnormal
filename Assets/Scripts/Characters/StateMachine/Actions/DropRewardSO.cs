...
public class DropReward : StateAction
{
	...
	{
		float weight = 0f;
		foreach (var drop in dropGroup.Drops)
		{
			weight += drop.ItemDropRate;
		}
		float dropDice = Random.Range(0, weight);
		...
		GameObject collectibleItem = ObjectPool.Instantiate(itemPrefab,
